...
use core::webhook_server::{
  validate_hmac_sha1, validate_hmac_sha256, WebhookServerBuilder, WebhookServerConfig,
};
use hex;
use hmac::{Hmac, Mac};
use sha1::Sha1;
...
fn test_webhook_lifecycle_operations() {
  ...
  assert!(trigger_manager
    .register_webhook_trigger("github-workflow", trigger1)
    .is_ok());
  assert!(trigger_manager
    .register_webhook_trigger("gitlab-workflow", trigger2)
    .is_ok());
...
fn test_webhook_validation_configurations() {
  ...
  let validation1 = WebhookValidation::new().with_secret("secret-only".to_string());
  ...
  assert_eq!(
    validation2.signature_header.as_ref().unwrap(),
    "X-Custom-Signature"
  );
