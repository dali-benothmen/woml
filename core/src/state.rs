...
use crate::database::{AsyncDatabase, Database};
use crate::error::{CoreError, CoreResult};
use crate::models::{RunStatus, StepResult, WorkflowDefinition, WorkflowRun};
use chrono::Utc;
...
impl StateManager {
  ...
  pub fn cleanup_completed_runs(&mut self) -> CoreResult<()> {
    let completed_runs: Vec<Uuid> = self
      .active_runs
      .iter()
...
impl AsyncStateManager {
  ...
  pub async fn update_run_with_steps(
    &self,
    run_id: &Uuid,
    completed_steps: &[StepResult],
  ) -> CoreResult<()> {
