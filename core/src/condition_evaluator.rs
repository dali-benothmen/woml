...
impl ConditionEvaluator {
      ...
      match field_type {
        "output" => {
          if parts.len() >= 5 {
            ...
            return Ok(ParsedCondition::StepOutput(
              step_id.to_string(),
              "".to_string(),
            ));
          }
  ...
  fn evaluate_parsed_condition(&self, condition: &ParsedCondition) -> CoreResult<ConditionResult> {
    match condition {
      ParsedCondition::Boolean(value) => Ok(ConditionResult::success(*value)),
  ...
  fn evaluate_comparison(&self, left: &str, op: &str, right: &str) -> CoreResult<bool> {
      ...
      match parsed {
        ...
        _ => {
          return Err(CoreError::Validation(
            "Unsupported left operand in comparison".to_string(),
          ))
        }
      }
    } else {
    ...
    let right_value = if right.starts_with("ctx.") {
      ...
      match parsed {
        ...
        _ => {
          return Err(CoreError::Validation(
            "Unsupported right operand in comparison".to_string(),
          ))
        }
      }
    } else {
  ...
  fn value_to_number(&self, value: &Value) -> CoreResult<f64> {
    match value {
      Value::Number(n) => n
        .as_f64()
        .ok_or_else(|| CoreError::Validation("Invalid number value".to_string())),
      Value::String(s) => s
        .parse::<f64>()
        .map_err(|_| CoreError::Validation(format!("Cannot convert string '{}' to number", s))),
