  ...
  pub async fn stop(&mut self) -> CoreResult<()> {
    ...
    if let Some(handle) = self.server_handle.take() {
      match tokio::time::timeout(self.config.graceful_shutdown_timeout, handle).await {
        ...
        Err(_) => {
          log::warn!(
            "Webhook server shutdown timed out after {:?}",
...
async fn webhook_handler(
  ...
  if let Err(signature_error) =
    validate_webhook_signature(&webhook_request, &body.to_vec(), &trigger_manager).await
  {
    log::error!(
...
async fn handle_webhook_request(
  ...
  let trigger_manager_guard = trigger_manager
    .lock()
  ...
  let mut state_manager_guard = state_manager
    .lock()
  ...
  log::info!(
    "Created workflow run {} for webhook-triggered workflow {}",
    run_id,
    workflow_id
  );
...
async fn validate_webhook_signature(
  ...
  let (trigger, _workflow_id) = trigger_manager_guard
    .get_webhook_trigger(&request.path)
    .ok_or_else(|| {
      CoreError::TriggerNotFound(format!(
        "No webhook trigger found for path: {}",
        request.path
      ))
    })?;
  ...
  let signature_header = validation
    .signature_header
    .as_deref()
    .unwrap_or("x-hub-signature-256");
  let signature_algorithm = validation
    .signature_algorithm
    .as_deref()
    .unwrap_or("sha256");

  // Get the signature from headers
  let received_signature = request.headers.get(signature_header).ok_or_else(|| {
    CoreError::InvalidTrigger(format!("Missing signature header: {}", signature_header))
  })?;
  ...
  match signature_algorithm.to_lowercase().as_str() {
    ...
    _ => Err(CoreError::InvalidTrigger(format!(
...
mod tests {
  ...
  fn test_webhook_trigger_registration() {
    ...
    assert!(
      result.is_ok(),
      "Webhook trigger registration should succeed"
    );
  ...
  fn test_webhook_signature_validation_with_trigger() {
    ...
    trigger_manager
      .register_webhook_trigger("secure-workflow", trigger)
      .unwrap();

    // Test that we can retrieve the trigger configuration
    let (retrieved_trigger, workflow_id) = trigger_manager
      .get_webhook_trigger("/webhook/secure")
      .unwrap();
    ...
    assert_eq!(
      validation.signature_header.as_ref().unwrap(),
      "x-hub-signature-256"
    );
