...
impl TriggerManager {
  ...
  pub fn register_webhook_trigger(
    ...
    log::info!(
      "Registering webhook trigger for workflow: {} at path: {}",
      workflow_id,
      trigger.path
    );
    ...
    self
      .webhook_triggers
      .insert(path.clone(), (trigger, workflow_id.to_string()));

    log::info!(
      "Successfully registered webhook trigger for workflow: {} at path: {}",
      workflow_id,
      path
    );
    Ok(())
  ...
  pub fn handle_webhook_request(
    ...
    let (trigger, workflow_id) = self.webhook_triggers.get(&request.path).ok_or_else(|| {
    ...
    log::info!(
      "Webhook request validated, triggering workflow: {}",
      workflow_id
    );
  ...
  fn validate_webhook(
    ...
    if let Some(required_fields) = &validation.required_fields {
      if let Some(body) = &request.body {
        ...
        return Err(CoreError::InvalidTrigger(
          "Request body is required for field validation".to_string(),
        ));
      }
    }
  ...
  pub fn get_webhook_trigger(&self, path: &str) -> Option<(&WebhookTrigger, &String)> {
    self
      .webhook_triggers
      .get(path)
      .map(|(trigger, workflow_id)| (trigger, workflow_id))
  }
}
