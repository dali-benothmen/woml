...
mod tests {
  use super::*;
  use crate::bridge::{create_run, execute_step, get_run_status, register_workflow};
  use crate::database::Database;
  use crate::models::{
    RetryConfig, RunStatus, StepDefinition, StepResult, StepStatus, TriggerDefinition,
  ...
  fn test_workflow_definition_serialization() {
    let workflow = WorkflowDefinition {
      ...
      triggers: vec![TriggerDefinition::Manual],
      created_at: Utc::now(),
      updated_at: Utc::now(),
    };
    ...
    assert!(
      ...
      "Workflow should deserialize from JSON"
    );

    let deserialized = deserialized_result.unwrap();
    assert_eq!(
      deserialized.id, workflow.id,
      "Deserialized workflow should have same ID"
    );
    assert_eq!(
      deserialized.name, workflow.name,
      "Deserialized workflow should have same name"
    );
    assert_eq!(
      deserialized.steps.len(),
      workflow.steps.len(),
      "Deserialized workflow should have same number of steps"
    );
  }
  ...
  fn test_workflow_run_serialization() {
    ...
    let json = json_result.unwrap();
    assert!(
      json.contains(&run_id.to_string()),
      "JSON should contain run ID"
    );
    assert!(
      json.contains("test-workflow"),
      "JSON should contain workflow ID"
    ...
    assert!(
      deserialized_result.is_ok(),
      "Workflow run should deserialize from JSON"
    );

    let deserialized = deserialized_result.unwrap();
    assert_eq!(
      deserialized.id, run.id,
      "Deserialized run should have same ID"
    );
    assert_eq!(
      deserialized.workflow_id, run.workflow_id,
      "Deserialized run should have same workflow ID"
    );
    assert!(
      matches!(deserialized.status, RunStatus::Pending),
      "Deserialized run should have same status"
    );
  }
  ...
  fn test_step_result_serialization() {
    ...
    assert!(
      deserialized_result.is_ok(),
      "Step result should deserialize from JSON"
    );

    let deserialized = deserialized_result.unwrap();
    assert_eq!(
      deserialized.step_id, step_result.step_id,
      "Deserialized step result should have same step ID"
    );
    assert!(
      matches!(deserialized.status, StepStatus::Completed),
      "Deserialized step result should have same status"
    );
  }
  ...
  fn test_state_manager_workflow_registration() {
    ...
    assert!(
      state_manager_result.is_ok(),
      "State manager should be created successfully"
    );
    ...
    let workflow = WorkflowDefinition {
      ...
      triggers: vec![TriggerDefinition::Webhook {
    ...
    assert!(
      retrieved_workflow_result.is_ok(),
      "Workflow retrieval should succeed"
    );

    let retrieved_workflow = retrieved_workflow_result.unwrap();
    assert!(
      retrieved_workflow.is_some(),
      "Retrieved workflow should exist"
    );

    let retrieved = retrieved_workflow.unwrap();
    assert_eq!(
      retrieved.id, workflow.id,
      "Retrieved workflow should have same ID"
    );
    assert_eq!(
      retrieved.name, workflow.name,
      "Retrieved workflow should have same name"
    );
    assert_eq!(
      retrieved.steps.len(),
      workflow.steps.len(),
      "Retrieved workflow should have same number of steps"
    );
    ...
    assert_eq!(
      run.workflow_id, "test-workflow",
      "Run should have correct workflow ID"
    );
    assert!(
      matches!(run.status, RunStatus::Pending),
      "Run should have pending status"
    );
  ...
  fn test_napi_bridge_functions() {
    ...
    assert!(
      ...
      "N-API workflow registration should succeed: {}",
      register_result.message
    );

    let payload_json = r#"{"test": "data", "timestamp": 1234567890}"#;
    let create_result = create_run(
      "test-workflow-napi".to_string(),
      payload_json.to_string(),
      db_path.to_string(),
    );
    assert!(
      create_result.success,
      "N-API run creation should succeed: {}",
    ...
    assert!(
      ...
      "N-API status retrieval should succeed: {}",
    ...
    assert!(
      step_result.success,
      "N-API step execution should succeed: {}",
      step_result.message
    );
    assert!(
      step_result.result.is_some(),
      "Step result should be returned"
    );
