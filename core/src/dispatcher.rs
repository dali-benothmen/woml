...
use crate::models::{RunStatus, StepResult, StepStatus, WorkflowDefinition, WorkflowRun};
...
impl Dispatcher {
  ...
  pub async fn start(&mut self) -> Result<(), CoreError> {
    log::info!(
      "Starting job dispatcher with {} workers",
      ...
      loop {
        ...
        if let Some(mut job) = job {
          ...
          .unwrap_or_else(|e| {
            ...
            (
              Err(CoreError::Internal("Worker task panicked".to_string())),
              dummy_job,
            )
          });
          ...
          tokio::task::spawn_blocking(move || {
            if let Ok(step_result) = result {
              ...
              if let Err(e) =
                Self::process_job_result_internal(&state_manager_clone, &job_back, &step_result)
              {
              ...
              // Handle job failure
              if let Err(e) =
                Self::handle_job_failure_internal(&state_manager_clone, &mut job_back, &error)
              {
          ...
          // Update statistics
          {
            ...
            let total_time = stats_guard.average_processing_time_ms
              * (stats_guard.total_jobs_processed - 1)
              + processing_time;
            ...
            .filter(|job| {
              if let Some(started_at) = running.get(&job.id) {
                ...
                elapsed.num_milliseconds() as u64
                  > job.timeout_ms.unwrap_or(config.worker_timeout_ms)
              } else {
                false
              }
            })
  ...
  pub async fn scale_workers(&self) -> Result<(), CoreError> {
    ...
    if queue_depth > active_workers * 2 && total_workers < self.config.max_workers {
      log::info!(
        "Scaling up workers: queue_depth={}, active_workers={}",
        queue_depth,
        active_workers
      );
    }
    ...
    if queue_depth == 0 && total_workers > self.config.min_workers {
      log::info!(
        "Scaling down workers: queue_depth={}, total_workers={}",
        queue_depth,
        total_workers
      );
    }
  ...
  pub fn process_job_result(&self, job: &Job, step_result: &StepResult) -> Result<(), CoreError> {
    log::info!(
      "Processing result for job: {} (step: {})",
      job.id,
      job.step_name
    );
  ...
  fn update_workflow_state(
    ...
    rt.block_on(async {
      ...
      log::debug!(
        "Updated workflow state for run: {} step: {}",
        run_id,
        step_result.step_id
      );
      Ok::<(), CoreError>(())
  ...
  fn check_workflow_completion(&self, workflow_id: &str, run_id: &Uuid) -> Result<(), CoreError> {
    ...
    rt.block_on(async {
      ...
      let all_steps_completed = workflow.steps.iter().all(|step| {
        completed_steps
          .iter()
          .any(|result| result.step_id == step.id)
      });

      if all_steps_completed {
        let has_failures = completed_steps
          .iter()
        ...
        let error_message = if has_failures {
          let failed_steps: Vec<_> = completed_steps
            .iter()
  ...
  fn determine_next_steps(&self, workflow_id: &str, run_id: &uuid::Uuid) -> Result<(), CoreError> {
    ...
    rt.block_on(async {
      ...
      let ready_steps: Vec<_> = workflow
        .steps
        .iter()
        .filter(|step| {
          ...
          step
            .depends_on
            .iter()
            .all(|dep_id| completed_step_ids.contains(dep_id))
        })
        .collect();

      if !ready_steps.is_empty() {
        log::info!(
          "Steps ready for execution in workflow {} run {}: {:?}",
          workflow_id,
          run_id,
  ...
  pub async fn get_workflow_run_status(
    &self,
    run_id: &str,
  ) -> Result<Option<RunStatus>, CoreError> {
    let run_uuid = uuid::Uuid::parse_str(run_id)
      .map_err(|e| CoreError::Validation(format!("Invalid run ID: {}", e)))?;
  ...
  pub async fn get_workflow_completed_steps(
    &self,
    run_id: &str,
  ) -> Result<Vec<StepResult>, CoreError> {
    let run_uuid = uuid::Uuid::parse_str(run_id)
      .map_err(|e| CoreError::Validation(format!("Invalid run ID: {}", e)))?;
  ...
  fn handle_job_failure_internal(
    ...
    rt.block_on(async {
      ...
      if job.can_retry() {
        ...
        Self::check_workflow_completion_internal(
          &mut state_manager_guard,
          &workflow_id,
          &run_uuid,
        )?;
      }
...
mod tests {
  ...
  fn test_job_execution_flow() {
    let state_manager = Arc::new(Mutex::new(
      StateManager::new("test_job_execution_flow.db").unwrap(),
    ));
    let config = WorkerPoolConfig::default();
  ...
  fn test_job_result_processing_flow() {
    let state_manager = Arc::new(Mutex::new(
      StateManager::new("test_job_result_processing_flow.db").unwrap(),
    ));
    let config = WorkerPoolConfig::default();
    let mut dispatcher = Dispatcher::new(config, state_manager);
  ...
  fn test_job_error_handling_flow() {
    let state_manager = Arc::new(Mutex::new(
      StateManager::new("test_job_error_handling_flow.db").unwrap(),
    ));
    let config = WorkerPoolConfig::default();
    let mut dispatcher = Dispatcher::new(config, state_manager);
