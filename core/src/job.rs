...
use crate::models::{StepDefinition, StepResult, WorkflowDefinition, WorkflowRun};
...
impl Job {
  ...
  pub fn from_workflow_step(
    ...
    step.validate().map_err(|e| CoreError::InvalidWorkflow(e))?;
  ...
  pub fn create_workflow_jobs(
    ...
    log::info!(
      "Creating jobs for workflow: {} run: {}",
      workflow.id,
      run.id
    );
  ...
  pub fn create_step_jobs(
    ...
    log::info!(
      "Creating jobs for steps: {:?} in workflow: {} run: {}",
      step_ids,
      workflow.id,
      run.id
    );
    ...
    log::info!(
      "Created {} jobs for specified steps in workflow: {}",
      jobs.len(),
      workflow.id
    );
    Ok(jobs)
  ...
  fn add_workflow_context(
    ...
    job.add_context(
      "workflow_created_at".to_string(),
      serde_json::Value::String(workflow.created_at.to_rfc3339()),
    );
    job.add_context(
      "workflow_updated_at".to_string(),
      serde_json::Value::String(workflow.updated_at.to_rfc3339()),
    );

    job.add_context(
      "run_started_at".to_string(),
      serde_json::Value::String(run.started_at.to_rfc3339()),
    );
    job.add_context(
      "run_status".to_string(),
      serde_json::Value::String(run.status.as_str().to_string()),
    );
    ...
    if let Some(step_index) = workflow.steps.iter().position(|s| s.id == job.step_name) {
      ...
      job.add_context(
        ...
        serde_json::Value::Number(workflow.steps.len().into()),
      );
    }
  ...
  pub fn can_retry(&self) -> bool {
    self.state == JobState::Failed && self.metadata.attempt_count < self.retry_config.max_attempts
  }
...
impl JobQueue {
  ...
  pub fn cleanup(&mut self) {
    self
      .jobs
      .retain(|job| job.state != JobState::Completed && job.state != JobState::Failed);
  }
}
...
mod tests {
  use super::*;
  use crate::models::{
    RetryConfig as ModelsRetryConfig, RunStatus, StepDefinition, StepStatus, TriggerDefinition,
    WorkflowDefinition,
  };
  use chrono::Utc;
  use uuid::Uuid;
  ...
  fn test_from_workflow_step_with_dependencies() {
    ...
    assert!(job
      .dependencies
      .contains(&format!("{}:{}:step-1", workflow.id, run.id)));
  }
  ...
  fn test_workflow_context() {
    ...
    assert_eq!(
      job.get_context("workflow_name").unwrap(),
      &serde_json::Value::String(workflow.name)
    );
    assert_eq!(
      job.get_context("step_index").unwrap(),
      &serde_json::Value::Number(0.into())
    );
    assert_eq!(
      job.get_context("total_steps").unwrap(),
