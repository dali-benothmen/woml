...
use crate::models::{
  ConditionResult, ConditionType, ControlFlowBlock, ParallelGroupStatus, ParallelStepGroup,
  RunStatus, StepDefinition, StepResult, StepStatus, WorkflowDefinition, WorkflowRun,
};
...
impl WorkflowExecutionState {
  ...
  ) -> CoreResult<WorkflowExecutionState> {
...
impl WorkflowStateMachine {
  ...
  pub fn new(state_manager: Arc<Mutex<StateManager>>, workflow_id: String, run_id: Uuid) -> Self {
    Self {
      state_manager,
      workflow_id,
      run_id,
  ...
  pub fn initialize(&mut self) -> CoreResult<()> {
    log::info!(
      "Initializing workflow state machine for run: {}",
      self.run_id
    );
    ...
    let new_state = self
      .execution_state
      .transition_to(WorkflowExecutionState::Running)?;
    self.execution_state = new_state;

    log::info!(
      "Workflow state machine initialized with {} steps",
      self.total_steps
    );
    Ok(())
  }
  ...
  fn initialize_control_flow_blocks(&mut self, workflow: &WorkflowDefinition) -> CoreResult<()> {
          ...
          match condition_type {
            ConditionType::If => {
              ...
              if let Some(block_id) = &step.control_flow_block {
                ...
                let block =
                  ControlFlowBlock::new(block_id.clone(), condition_type.clone(), step.id.clone());
    ...
    log::debug!(
      "Initialized {} control flow blocks",
      self.control_flow_blocks.len()
    );
    Ok(())
  }
    ...
    for step in &workflow.steps {
      if step.is_parallel() {
        let group_id = step
          ...
          .ok_or_else(|| CoreError::Validation("Parallel step without a group ID".to_string()))?;

        let group = self
          .parallel_groups
          .entry(group_id.clone())
          .or_insert_with(|| ParallelStepGroup::new(group_id.clone(), Vec::new()));
  ...
  pub fn evaluate_step_condition(&mut self, step_id: &str) -> CoreResult<ConditionResult> {
    ...
    let condition_expr = step_state.step.get_condition_expression().ok_or_else(|| {
      CoreError::Validation(format!(
        "Step {} requires condition but has no expression",
        step_id
      ))
    })?;
  ...
  fn skip_until_control_flow_end(&mut self, current_step_id: &str) -> CoreResult<()> {
    ...
    let block_id = current_step
      .get_control_flow_block_id()
  ...
  pub fn mark_step_running(&mut self, step_id: &str) -> CoreResult<()> {
    if let Some(step_state) = self.step_states.get_mut(step_id) {
      ...
      Err(CoreError::StepNotFound(format!(
        "Step not found: {}",
        step_id
      )))
    }
  ...
  pub fn pause(&mut self) -> CoreResult<()> {
    let new_state = self
      .execution_state
      .transition_to(WorkflowExecutionState::Paused)?;
    self.execution_state = new_state;
  ...
  pub fn cancel(&mut self, reason: Option<String>) -> CoreResult<()> {
    let new_state = self
      .execution_state
      .transition_to(WorkflowExecutionState::Cancelled)?;
    self.execution_state = new_state;
    log::info!(
  ...
  pub fn execute_completion_hooks(
    &self,
    context: &crate::models::WorkflowCompletionContext,
  ) -> CoreResult<()> {
    log::info!(
      "Executing completion hooks for workflow: {} run: {}",
      context.workflow_id,
      context.run_id
    );
  ...
  pub fn finalize_completion(&mut self, error_message: Option<String>) -> CoreResult<()> {
    log::info!(
      "Finalizing workflow completion for: {} run: {}",
      self.workflow_id,
      self.run_id
    );
    ...
    let completion_context =
      self.create_completion_context(final_status.clone(), error_message.clone())?;
    ...
    log::info!(
      "Workflow {} finalized with status: {:?}",
      self.workflow_id,
      final_status
    );
    Ok(())
  ...
  pub fn execute_parallel_group(
  ...
  ) -> CoreResult<Vec<StepResult>> {
    log::info!(
      "Executing parallel group: {} with {} steps",
      group.group_id,
      group.step_ids.len()
    );
    ...
    log::info!(
      "Parallel group {} completed with {} results",
      group.group_id,
      results.len()
    );
    Ok(results)
  ...
  pub fn aggregate_parallel_results(
    &self,
    ...
    aggregated.insert(
      ...
      serde_json::Value::Number((success_count + failure_count).into()),
    );
  ...
  pub fn handle_parallel_failures(
    &mut self,
    group: &ParallelStepGroup,
    failures: Vec<String>,
  ) -> CoreResult<()> {
    log::warn!(
      "Handling {} failures in parallel group: {}",
      failures.len(),
      group.group_id
    );
...
mod tests {
  use super::*;
  use crate::models::{RunStatus, StepDefinition, TriggerDefinition, WorkflowDefinition};
  use chrono::Utc;
  use uuid::Uuid;
    ...
    WorkflowDefinition {
      ...
      control_flow_blocks: vec![
        ControlFlowBlock {
          ...
          conditions: vec![ConditionType::If(ConditionResult::True)],
        },
        ControlFlowBlock {
          id: "block-2".to_string(),
          conditions: vec![ConditionType::If(ConditionResult::False)],
        },
      ],
      created_at: Utc::now(),
      updated_at: Utc::now(),
  ...
  fn test_state_machine_creation() {
    let state_manager = Arc::new(Mutex::new(
      crate::state::StateManager::new(":memory:").unwrap(),
    ));
    let run_id = Uuid::new_v4();
    let state_machine =
      WorkflowStateMachine::new(state_manager, "test-workflow".to_string(), run_id);

    assert_eq!(state_machine.workflow_id, "test-workflow");
    assert_eq!(state_machine.run_id, run_id);
    assert_eq!(
      state_machine.execution_state,
