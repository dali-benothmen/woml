...
use crate::{
  ...
  state::{AsyncStateManager, StateManager},
...
macro_rules! with_shared_bridge {
  ($db_path:expr, $success_result:expr, $failure_result:expr, $operation:expr) => {
    match get_shared_bridge($db_path) {
      Ok(bridge) => match $operation(bridge) {
...
impl Bridge {
  ...
  pub fn register_workflow(&self, workflow_json: &str) -> CoreResult<()> {
    ...
    let workflow: WorkflowDefinition =
      serde_json::from_str(workflow_json).map_err(|e| CoreError::Serialization(e))?;
    ...
    let trigger_ids = self
      .trigger_executor
      .register_workflow_triggers(&workflow.id, &workflow)?;

    log::info!(
      "Successfully registered workflow: {} with {} triggers: {:?}",
      workflow.id,
      trigger_ids.len(),
      trigger_ids
    );
  ...
  pub fn register_webhook_trigger(&self, workflow_id: &str, trigger_json: &str) -> CoreResult<()> {
    log::info!(
      "Registering webhook trigger for workflow: {} with config: {}",
      workflow_id,
      trigger_json
    );

    let trigger: crate::triggers::WebhookTrigger =
      serde_json::from_str(trigger_json).map_err(|e| CoreError::Serialization(e))?;

    trigger.validate()?;

    let mut trigger_manager = self
      .trigger_manager
      .lock()
    ...
    log::info!(
      "Successfully registered webhook trigger for workflow: {}",
      workflow_id
    );
    Ok(())
  }
  ...
  pub fn get_webhook_triggers(&self) -> CoreResult<String> {
    ...
    let triggers_json =
      serde_json::to_string(&triggers).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub fn create_run(&self, workflow_id: &str, payload_json: &str) -> CoreResult<String> {
    log::info!(
      "Creating run for workflow: {} with payload: {}",
      workflow_id,
      payload_json
    );

    let payload: serde_json::Value =
      serde_json::from_str(payload_json).map_err(|e| CoreError::Serialization(e))?;
    ...
    log::info!(
      "Successfully created run: {} for workflow: {}",
      run_id,
      workflow_id
    );
    Ok(run_id.to_string())
  ...
  pub fn get_run_status(&self, run_id: &str) -> CoreResult<String> {
    ...
    let run_uuid = uuid::Uuid::parse_str(run_id).map_err(|e| CoreError::UuidParse(e))?;
    ...
    let result = serde_json::to_string(&status_json).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub fn execute_step(&self, run_id: &str, step_id: &str) -> CoreResult<String> {
    ...
    let result_json = serde_json::to_string(&result).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub fn execute_job(&self, job: &Job) -> CoreResult<String> {
    ...
    let _run_uuid = Uuid::parse_str(&job.run_id).map_err(|e| CoreError::UuidParse(e))?;
  ...
  pub fn execute_webhook_trigger(&self, request_json: &str) -> CoreResult<String> {
    ...
    let request: crate::triggers::WebhookRequest =
      serde_json::from_str(request_json).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub fn execute_manual_trigger(
    ...
    log::info!(
      "Executing manual trigger for workflow: {} with payload: {}",
      workflow_id,
      payload_json
    );
    ...
    let result = self
      .trigger_executor
      .execute_manual_trigger(workflow_id, payload)?;
  ...
  pub fn get_trigger_stats(&self) -> CoreResult<String> {
    ...
    // Serialize the result
    let stats_json = serde_json::to_string(&stats).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub fn unregister_workflow_triggers(&self, workflow_id: &str) -> CoreResult<()> {
    ...
    self
      .trigger_executor
      .unregister_workflow_triggers(workflow_id)?;

    log::info!(
      "Successfully unregistered triggers for workflow: {}",
      workflow_id
    );
    Ok(())
  }
  ...
  pub fn get_workflow_run_status(
    &self,
    run_id: &str,
  ) -> CoreResult<Option<crate::models::RunStatus>> {
  ...
  pub fn get_workflow_completed_steps(
    &self,
    run_id: &str,
  ) -> CoreResult<Vec<crate::models::StepResult>> {
  ...
  pub fn execute_workflow_steps(&self, run_id: &str, workflow_id: &str) -> CoreResult<String> {
    log::info!(
      "Executing workflow steps for run: {} workflow: {}",
      run_id,
      workflow_id
    );
    ...
    let step_orchestrator =
      crate::step_orchestrator::StepOrchestrator::new(self.state_manager.clone());
    ...
    match step_orchestrator.start_step_execution(&run_uuid, workflow_id) {
      ...
      Err(error) => {
        log::error!(
          "Failed to execute workflow steps for run {}: {}",
          run_id,
          error
        );
        Err(error)
...
impl AsyncBridge {
  ...
  pub fn new(db_path: &str) -> CoreResult<Self> {
    ...
    let job_dispatcher = Arc::new(TokioMutex::new(Dispatcher::new(
      dispatcher_config,
      async_state_manager.clone(),
    )));
  ...
  pub async fn execute_job(&self, job_json: &str) -> CoreResult<String> {
    ...
    let job: Job = serde_json::from_str(job_json).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub async fn register_webhook_trigger(
    &self,
    workflow_id: &str,
    trigger_json: &str,
  ) -> CoreResult<()> {
...
pub async fn register_workflow_async(
  workflow_json: String,
  db_path: String,
) -> napi::Result<WorkflowRegistrationResult> {
  match get_shared_async_bridge(&db_path).await {
    Ok(bridge) => match bridge.register_workflow(&workflow_json).await {
...
#[napi]
pub fn register_webhook_trigger(
  workflow_id: String,
  trigger_json: String,
  db_path: String,
...
pub async fn create_run_async(
  ...
  payload_json: String,
  db_path: String,
) -> napi::Result<RunCreationResult> {
  match get_shared_async_bridge(&db_path).await {
    Ok(bridge) => match bridge.create_run(&workflow_id, &payload_json).await {
...
pub async fn execute_step_async(
  ...
  match get_shared_async_bridge(&db_path).await {
    Ok(bridge) => match bridge.execute_step(&run_id, &step_id).await {
...
pub fn execute_job(job_json: String, db_path: String) -> JobExecutionResult {
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.execute_job(&job) {
      Ok(result_json) => {
...
pub async fn execute_job_async(
  job_json: String,
  db_path: String,
) -> napi::Result<JobExecutionResult> {
  log::info!("Executing job with context (async): {}", job_json);

  match get_shared_async_bridge(&db_path).await {
    Ok(bridge) => match bridge.execute_job(&job_json).await {
      Ok(result_json) => {
        let result: serde_json::Value = match serde_json::from_str(&result_json) {
          Ok(result) => result,
          Err(_) => {
...
pub fn get_job_status(job_id: String, db_path: String) -> JobStatusResult {
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.get_job_status(&job_id) {
      Ok(status) => {
...
pub fn cancel_job(job_id: String, db_path: String) -> JobCancellationResult {
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.cancel_job(&job_id) {
      Ok(cancelled) => JobCancellationResult {
        success: true,
        job_id: Some(job_id),
        cancelled,
...
pub fn get_workflow_run_status(run_id: String, db_path: String) -> WorkflowRunStatusResult {
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.get_workflow_run_status(&run_id) {
      Ok(status) => {
        let status_str = match status {
          Some(s) => format!("{:?}", s),
          None => "not_found".to_string(),
...
pub fn get_workflow_completed_steps(run_id: String, db_path: String) -> WorkflowStepsResult {
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.get_workflow_completed_steps(&run_id) {
      Ok(steps) => {
...
pub fn execute_webhook_trigger(request_json: String, db_path: String) -> TriggerExecutionResult {
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.execute_webhook_trigger(&request_json) {
      Ok(result_json) => {
        let result: serde_json::Value = match serde_json::from_str(&result_json) {
          Ok(result) => result,
          Err(_) => {
        ...
        TriggerExecutionResult {
          ...
          message: result["message"]
            .as_str()
            .unwrap_or("Webhook trigger executed successfully")
            .to_string(),
        }
      }
      Err(e) => TriggerExecutionResult {
        success: false,
        run_id: None,
        workflow_id: None,
...
pub fn execute_manual_trigger(
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.execute_manual_trigger(&workflow_id, &payload_json) {
      Ok(result_json) => {
        let result: serde_json::Value = match serde_json::from_str(&result_json) {
          Ok(result) => result,
          Err(_) => {
        ...
        TriggerExecutionResult {
          ...
          message: result["message"]
            .as_str()
            .unwrap_or("Manual trigger executed successfully")
            .to_string(),
        }
      }
      Err(e) => TriggerExecutionResult {
...
#[napi]
pub fn unregister_workflow_triggers(
  workflow_id: String,
  db_path: String,
...
pub fn execute_workflow_hook(
  ...
  match get_shared_bridge(&db_path) {
    Ok(bridge) => match bridge.execute_workflow_hook(&hook_type, &context_json, &workflow_id) {
      Ok(result) => HookExecutionResult {
        success: true,
