...
use crate::models::{StepResult, WorkflowRun};
...
impl Context {
  ...
  pub fn new(
    ...
    if step_name.is_empty() {
      return Err(CoreError::Validation(
        "step_name cannot be empty".to_string(),
      ));
    }
  ...
  pub fn calculate_complexity_score(&self) -> u8 {
    ...
    if self.steps.len() > 50 {
      score += 2;
    } else if self.steps.len() > 10 {
      score += 1;
    }

    if self.has_deep_nesting(&self.payload, 0) {
      score += 1;
    }
  ...
  fn has_deep_nesting(&self, value: &serde_json::Value, depth: u8) -> bool {
    ...
    match value {
      serde_json::Value::Object(map) => map.values().any(|v| self.has_deep_nesting(v, depth + 1)),
      serde_json::Value::Array(arr) => arr.iter().any(|v| self.has_deep_nesting(v, depth + 1)),
      _ => false,
    }
  }
  ...
  pub fn to_json(&self) -> Result<String, CoreError> {
    ...
    serde_json::to_string(&context_for_serialization).map_err(|e| CoreError::Serialization(e))
  }
  ...
  pub fn from_json(json: &str) -> Result<Self, CoreError> {
    let context: Context = serde_json::from_str(json).map_err(|e| CoreError::Serialization(e))?;
  ...
  pub fn to_json_value(&self) -> Result<serde_json::Value, CoreError> {
    serde_json::to_value(self).map_err(|e| CoreError::Serialization(e))
  }

  /// Get the size of the context in bytes
  pub fn size_bytes(&self) -> Result<usize, CoreError> {
    let json_string = serde_json::to_string(self).map_err(|e| CoreError::Serialization(e))?;
    Ok(json_string.len())
  }
...
mod tests {
  use super::*;
  use crate::models::{RunStatus, StepResult, StepStatus, WorkflowRun};
  use chrono::Utc;
  use uuid::Uuid;
