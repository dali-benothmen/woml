...
impl TriggerExecutor {
  ...
  pub fn execute_webhook_trigger(
    &self,
    ...
    let workflow_id = trigger_manager
      .get_workflow_id_for_webhook(&request.path)
      .ok_or_else(|| {
        CoreError::TriggerNotFound(format!("Webhook trigger not found: {}", request.path))
      })?
      .clone();
    ...
    log::info!(
      "Webhook trigger executed successfully for workflow: {}",
      workflow_id
    );
    Ok(result)
  }
  ...
  pub fn execute_manual_trigger(
    ...
    log::info!(
      "Manual trigger executed successfully for workflow: {}",
      workflow_id
    );
    Ok(result)
  }
  ...
  fn execute_workflow(
    ...
    log::info!(
      "Created workflow run: {} for workflow: {}",
      run_id,
      workflow_id
    );
    ...
    match self.create_and_submit_jobs(&workflow, &run_id, &payload) {
      Ok(job_count) => {
        log::info!(
          "Successfully submitted {} jobs for workflow run: {}",
          job_count,
          run_id
        );
      }
      Err(error) => {
        log::error!(
          "Failed to submit jobs for workflow run {}: {}",
          run_id,
          error
        );
    ...
    Ok(TriggerExecutionResult::success(
      run_id,
  ...
  fn create_and_submit_jobs(
    ...
    log::info!(
      "Successfully created and submitted {} jobs for workflow: {}",
      job_count,
      workflow.id
    );
    Ok(job_count)
  ...
  pub fn get_workflow_triggers(&self, workflow_id: &str) -> CoreResult<Vec<String>> {
    ...
    log::info!(
      "Retrieved {} triggers for workflow: {}",
      triggers.len(),
      workflow_id
    );
    Ok(triggers)
  ...
  pub fn register_workflow_triggers(
    ...
    log::info!(
      "Successfully registered {} triggers for workflow: {}",
      trigger_ids.len(),
      workflow_id
    );
    Ok(trigger_ids)
  ...
  pub fn unregister_workflow_triggers(&self, workflow_id: &str) -> CoreResult<()> {
    ...
    let webhook_paths: Vec<String> = trigger_manager
      .webhook_triggers
      .iter()
    ...
    for path in webhook_paths {
      ...
      log::info!(
        "Removed webhook trigger: {} for workflow: {}",
        path,
        workflow_id
      );
    }
    ...
    log::info!(
      "Successfully unregistered all triggers for workflow: {}",
      workflow_id
    );
    Ok(())
  }
...
mod tests {
  use super::*;
  use crate::models::{StepDefinition, TriggerDefinition, WorkflowDefinition};
  use chrono::Utc;
