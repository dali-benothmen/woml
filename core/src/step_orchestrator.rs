...
use crate::models::{StepResult, StepStatus, WorkflowDefinition, WorkflowRun};
use crate::state::StateManager;
use crate::workflow_state_machine::{WorkflowExecutionState, WorkflowStateMachine};
use chrono::Utc;
use log;
use serde_json;
...
impl StepOrchestrator {
  ...
  pub fn start_step_execution(&self, run_id: &Uuid, workflow_id: &str) -> CoreResult<()> {
    log::info!(
      "Starting step execution for run: {} workflow: {}",
      run_id,
      workflow_id
    );

    let mut state_machine =
      WorkflowStateMachine::new(self.state_manager.clone(), workflow_id.to_string(), *run_id);
  ...
  fn execute_steps_with_state_machine(
    &self,
    mut state_machine: WorkflowStateMachine,
  ) -> CoreResult<()> {
    log::info!(
      "Executing steps using state machine for workflow: {}",
      state_machine.get_workflow_definition().unwrap().id
    );

    let workflow = state_machine
      .get_workflow_definition()
      .ok_or_else(|| {
        CoreError::Internal("Workflow definition not found in state machine".to_string())
      })?
      .clone();

    let run = state_machine
      .get_workflow_run()
    ...
    while !state_machine.check_workflow_completion()? {
      ...
      if !parallel_groups.is_empty() {
        ...
        for step_id in ready_steps {
          ...
          match self.execute_step_with_state_machine(
            &workflow,
            &run,
            &step_def,
            &completed_steps,
    ...
    if is_complete {
      ...
      let error_message = if state_machine.get_stats().failed_steps > 0 {
        let failed_steps: Vec<_> = state_machine
          .get_completed_steps()
          .iter()
    ...
    log::info!(
      "Workflow execution completed with state: {:?}, stats: {:?}",
      final_state,
      stats
    );
    Ok(())
  ...
  fn execute_step_with_state_machine(
    ...
    log::debug!(
      "Executing step with state machine: {} for run: {}",
      step_def.id,
      run.id
    );
    ...
    let output = match self.execute_via_bun(
      &step_def.id,
      &context_json,
      &workflow.id,
      &run.id.to_string(),
      ...
      Err(error) => {
        log::warn!(
          "Bun.js execution failed for step {}, falling back to simulation: {}",
          step_def.id,
          error
        );
  ...
  fn execute_via_bun(
    ...
    log::info!(
      "Step {} executed successfully via Bun.js simulation",
      step_name
    );
    Ok(simulated_result)
  }
