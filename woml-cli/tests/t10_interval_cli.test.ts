...
describe('T10 interval CLI boundary', () => {
  nativeTest(
    ...
    async () => {
      ...
      const exitCode = await runCli(
        ...
        io,
        {
          ...
          readSecret: async () => {
            throw new Error('Interval T10 must not read a secret.');
          },
      ...
      expect(stdout).toBe('');
      expect(stderr).toContain('Interval heartbeat every 5000ms next due at');
      expect(stderr).toContain('WOML runtime is ready with 1 registered trigger.');
