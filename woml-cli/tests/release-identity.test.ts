...
describe('WOML v1 release identity', () => {
  test('pins one public package and one private compiler at 1.0.4', async () => {
  ...
  test('accepts only the exact release tag', () => {
    expect(() => verifyReleaseTag('1.0.4', 'v1.0.4')).not.toThrow();
    expect(() => verifyReleaseTag('1.0.4', 'v1.0.1')).toThrow(
      'must exactly match woml version v1.0.4',
    );
    expect(() => verifyReleaseTag('1.0.4', '1.0.4')).toThrow(
      'must exactly match woml version v1.0.4',
    );
  });
