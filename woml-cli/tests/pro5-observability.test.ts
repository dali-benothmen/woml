...
describe('PRO5 frozen observability artifacts', () => {
  test('matches the reviewed golden snapshot, log, and Prometheus fixtures', async () => {
    ...
    expect(await observed.snapshot()).toEqual(
      await Bun.file(
        new URL('./fixtures/production-runtime/pro5-snapshot.v1.json', import.meta.url)
      ).json()
    );
    expect(JSON.parse(logs.at(-1)!)).toEqual(
      await Bun.file(
        new URL('./fixtures/production-runtime/pro5-log.v1.json', import.meta.url)
      ).json()
    );
    expect(await observed.prometheusMetrics()).toBe(
      await Bun.file(
        new URL('./fixtures/production-runtime/pro5-metrics.prom', import.meta.url)
      ).text()
    );
  });
...
describe('PRO5 observability HTTP surface', () => {
